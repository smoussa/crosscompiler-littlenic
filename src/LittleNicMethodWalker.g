tree grammar LittleNicMethodWalker;
...
@after {
	...
	:	^(VARDECL TYPE ID)
		{
			if (inMethod) {
				if (!globals.containsKey($ID.text)) {
					$Scope::scopeMap.put($ID.text, $TYPE.text);
					$Scope[-1]::scopeMap.put($ID.text, $TYPE.text);
				} else {
	...
	:	^(CALL ID argList[args]?)
		{
			...
			if (methods.containsKey($ID.text)) {
	...
	|	ID
		{
			...
			if ($type == null && globals.containsKey(id)) {
	        	$type = globals.get(id);
	        }
		}
	|	INTEGER
